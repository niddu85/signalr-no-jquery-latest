...
(function ($, window, undefined) {
      ...
      function configureStopReconnectingTimeout(connection) {
        ...
        if (!connection._.configuredStopReconnectingTimeout) {
          onReconnectTimeout = function onReconnectTimeout(connection) {
            var message = _signalR._.format(
              _signalR.resources.reconnectTimeout,
              connection.disconnectTimeout
            );
            connection.log(message);
            $(connection).triggerHandler(events.onError, [
              _signalR._.error(message, /* source */ "TimeoutException"),
            ]);
  ...
  _signalR._ = {
    ...
    transportError: function transportError(
      message,
      transport,
      source,
      context
  ...
  function validateTransport(requestedTransport, connection) {
    ...
    if ($.isArray(requestedTransport)) {
      ...
      !_signalR.transports[requestedTransport] &&
  ...
  _signalR.fn = _signalR.prototype = {
    ...
    start: function start(options, callback) {
      ...
      if (connection.state === _signalR.connectionState.connecting) {
        ...
        changeState(
          connection,
          _signalR.connectionState.disconnected,
          _signalR.connectionState.connecting
        ) === false
      ) {
      ...
      connection._.initHandler =
        _signalR.transports._logic.initHandler(connection);
          ...
          connection._.initHandler.start(
            ...
            function () {
              ...
              var isFirefox11OrGreater =
                  _signalR._.firefoxMajorVersion(window.navigator.userAgent) >=
                  11,
                asyncAbort = true;
              ...
              if (
                !changeState(
                  connection,
                  _signalR.connectionState.connecting,
                  _signalR.connectionState.connected
                )
              ) {
                connection.log(
      ...
      connection._.negotiateRequest = (function () {
        var res,
          ...
          negotiate = function negotiate(connection, onSuccess) {
            var url = _signalR.transports._logic.prepareQueryString(
              connection,
          ...
          callback = function callback(result) {
            ...
            if (res.availableTransports) {
              protocolError = _signalR._.error(
                resources.aspnetCoreSignalrServer
              );
            ...
            if (
              connection.negotiateRedirectSupportedProtocols.indexOf(
                res.ProtocolVersion
              ) !== -1
            ) {
              if (res.Error) {
                protocolError = _signalR._.error(
                  _signalR._.format(resources.errorFromServer, res.Error)
                );
                $(connection).triggerHandler(events.onError, [protocolError]);
                deferred.reject(protocolError);
                return;
                ...
                if (redirects === MAX_REDIRECTS) {
                  onFailed(
                    _signalR._.error(resources.errorRedirectionExceedsLimit),
                    connection
                  );
                  return;
                }
                ...
                connection._.redirectQs =
                  splitUrlAndQs.length === 2 ? splitUrlAndQs[1] : null;
                ...
                if (
                  ...
                  onFailed(
                    _signalR._.error(
                      resources.jsonpNotSupportedWithAccessToken
                    ),
                    connection
                  );
                  return;
...
(function ($, window, undefined) {
  ...
  transportLogic = signalR.transports._logic = {
    ...
    addQs: function addQs(url, qs) {
      ...
      if (
        (typeof qs === "undefined" ? "undefined" : _typeof(qs)) === "object"
      ) {
    ...
    getUrl: function getUrl(
      ...
      if (connection.transport && connection.accessToken) {
        if (
          ...
          url +=
            "&access_token=" +
            window.encodeURIComponent(connection.accessToken);
        }
      }
    ...
    maximizePersistentResponse: function maximizePersistentResponse(
      minPersistentResponse
    ) {
      return {
    ...
    tryInitialize: function tryInitialize(
      connection,
      persistentResponse,
      onInitialized
    ) {
    ...
    processMessages: function processMessages(
      ...
      if (minData) {
        ...
        if (data.Error) {
          ...
          connection.log(
            "Received an error message from the server: " + minData.E
          );
    ...
    handleParseFailure: function handleParseFailure(
      connection,
      result,
      error,
      onFailed,
...
(function ($, window, undefined) {
  var signalR = $.signalR,
    ...
    changeState = $.signalR.changeState,
    transportLogic = signalR.transports._logic,
    clearReconnectAttemptTimeout = function clearReconnectAttemptTimeout(
      connection
    ) {
  ...
  function isCallbackFromGeneratedHubProxy(callback) {
    ...
    return (
      ...
      callback
        .toString()
        .slice(0, 256)
        .indexOf("// Call the client hub method") >= 0
    );
  }
  ...
  hubProxy.fn = hubProxy.prototype = {
    ...
    on: function on(eventName, callback, callbackIdentity) {
      ...
      var that = this,
        ...
        isFromOldGeneratedHubProxy =
          !callbackIdentity && isCallbackFromGeneratedHubProxy(callback);
      ...
      for (var i = 0; i < callbackSpace.length; i++) {
        if (
          ...
          (isFromOldGeneratedHubProxy &&
            callbackSpace[i].isFromOldGeneratedHubProxy)
        ) {
      ...
      if (callbackSpace) {
        if (callback) {
          ...
          if (callbackRegistration) {
            ...
            for (
              var j = 0;
              j < callbackRegistration.eventHandlers.length;
              j++
            ) {
              $(that).unbind(
                makeEventName(eventName),
                callbackRegistration.eventHandlers[j]
              );
            }
